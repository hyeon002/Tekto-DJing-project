...
| `/settings` | 조그휠·노브 연결과 기본/베이스 보조/스크래치 음원 설정 |
...
- Setting에서 기본 음원을 변경하거나 베이스 보조 음원·스크래치 음원을 선택할 수 있습니다.
- 믹싱 화면의 제목·아티스트·커버는 선택한 앨범 정보입니다. 실제 재생 파일은 Setting의 기본 음원이며, 현재 앨범별로 다른 음악을 재생하지는 않습니다.
- 베이스 보조 음원은 기본 음원과 함께 재생되며 Bass 입력으로 음량을 조절합니다.
- 스크래치 음원은 재생 중 조그휠을 누르는 동안 대신 재생됩니다. 손을 떼면 기본 음원과 베이스 보조 음원을 멈춘 위치부터 이어 재생합니다.
- 보조 음원은 기본 제공되지 않습니다. 스크래치 음원이 없으면 터치해도 기본 음악을 유지합니다.
...
| 입력 | 도트 UI | 오디오 |
| --- | --- | --- |
| 노브 3채널 | 세 값에 따라 왼쪽 도트가 각각 변화 | 기존 코드상 Treble/Bass/Volume에 매핑 |
| 조그휠 터치 | 터치·회전값에 따른 도트 움직임은 미연동. 재생 중 자동 회전 | 기본 음원 ↔ 스크래치 음원 전환 |
| 화면 슬라이더 | 가운데 도트가 위아래로 이동 | 0.9~1.1배 재생 속도 |

노브의 Treble/Bass/Volume 매핑은 기존 코드에 남아 있는 구현입니다. 기능명세서에 명시된 노브 역할은 베이스 보조 음원 볼륨이며, 고음 조절 등 나머지 역할은 확정된 명세가 아닙니다.
...
- 현재 BLE 파서는 조그휠 `BLEMIDI_1`의 `T + 0/1`, 노브 `BLEMIDI_2`의 `K + 3개 값(각 0~100)` 형식을 가정합니다. 실제 펌웨어와 일치하는지 실기 검증이 필요합니다.
- 연결은 사용자가 버튼을 눌러 시작합니다. 브라우저의 Web Bluetooth 지원 및 보안 컨텍스트(HTTPS/localhost)가 필요합니다.
- 빌드·린트, 모의 BLE 입력 및 오디오 로직 테스트는 통과했습니다. 실제 기기 수신과 출력 음질 검증은 아직 완료되지 않았습니다.
