...
- `Settings.tsx` (`/settings`): 조그휠·노브 연결, 기본 음원·베이스 보조 음원·스크래치 음원 선택. USB 및 믹싱 시작 버튼은 없습니다.
...
- 노브: `BLEMIDI_2`의 `K + Treble/Bass/Volume` 세 값(각 0~100)을 왼쪽 도트 3개에 각각 반영합니다. 한 모듈의 세 채널을 가정합니다. 화면의 각 도트 영역을 가로로 조작하거나 키보드로 값을 바꿀 수도 있습니다.
- 조그휠: `BLEMIDI_1`의 `T + 0/1` 터치 신호를 오디오 전환에 사용합니다. 오른쪽 도트의 자동 회전은 실제 회전 센서값과 무관하며, 터치에 따른 도트 정지도 연결하지 않았습니다.
- 슬라이더: 가운데 패널을 세로로 조작하면 도트 위치와 재생 속도(0.9~1.1배)가 변합니다. 실제 슬라이더 기기는 연결하지 않습니다.
- 노브의 고음/전체 음량 처리는 기존 코드의 매핑입니다. 명세에 명시된 베이스 보조 음량 외 두 노브의 최종 역할은 미확정입니다.

## 오디오

`MixingAudio`는 기본 음원과 베이스 보조 음원을 같은 AudioContext 시계·위치로 재생합니다.
Bass는 보조 음원 볼륨, Treble은 고역 필터, Volume은 최종 출력 음량에 연결되어 있습니다.
스크래치 음원을 등록하면 재생 중 터치 On에서 120ms 크로스페이드로 전환하고, Off에서 기본/베이스를 멈춘 위치부터 재개합니다. 스크래치 음원이 없으면 기본 음악을 유지합니다.
베이스 파일이 없으면 Bass 조절로 실제 저음이 변하지 않습니다.
...
빌드·린트, 모의 BLE 패킷과 연결 해제, 조그 터치 후 재개 및 기본/베이스 동기화 테스트를 통과했습니다.
노브·슬라이더의 화면 입력에 따른 도트 변화도 확인했습니다. 실제 장치를 움직였을 때 수신되는 패킷과 음질은 실기 확인이 필요합니다.
